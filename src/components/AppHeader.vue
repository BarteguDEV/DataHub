...
const user = computed(() => state.user || { initials: '??', username: '—', role: '—' })
...
function handleLogout() {
  ...
  logout()
  // Używamy pełnego przeładowania, bo AppHeader (i cały layout)
  // jest niszczony przy przejściu na login — router.push
  // powodowałoby konflikt lifecycle Vue.
...
onMounted(() => {
  checkBackend()
  healthTimer = setInterval(checkBackend, 15000)
})
